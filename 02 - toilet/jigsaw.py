...
def CheckAndComplete():
    ...
    # checking unique
    # nums = [len(np.where(C == item)[0]) for item in unique_blocks]
    for item in unique_blocks:
        num = len(np.where(C == item)[0])
        if num != 1: return
    if lavatory not in C: return
    # shape of lavatory
    ys, xs = np.where(C == lavatory)
    ys_unique = np.unique(ys)
    xs_unique = np.unique(xs)
    if len(ys_unique) == 1:
        if len(xs_unique) > 1:
            if not line_array(xs_unique): return
        lavatorys = len(xs_unique)
    elif len(xs_unique) > 1: return
    elif not line_array(ys_unique): return 
    else: 
        lavatorys = len(ys_unique)
    if lavatorys > 4: return
    # find Perimeter
    B = np.ones_like(C)
    B[0, 0] = 0
    C[0, 0] = 1
    ...
    Scoring = -0.1 * (AreaReality - 4) ** 2 + 10
    Storage_s = 5 - 1 / lavatorys ** 0.5 * 2.5
    ...
    duplicated_key = AreaCell, Perimeter, lavatorys, B_cl_re.shape[0], B_cl_re.shape[1]
    ...
    img_all = np.zeros((max(500, cell_size * n_ex), cell_size * m_ex + 300, 3), dtype=np.uint8)
    img_all[:] = 255
    ...
    y += y_step
    result_str += ',%d' % lavatorys
    cv2.putText(img, 'Lavatory Table:', (x1, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1)
    cv2.putText(img, '%d' % lavatorys, (x2, y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 0), 1)
    y += y_step
    ...
    if success_number == 1:
        ...
        with open(csvName, 'wt') as f:
            f.write('No,Area Cell,Area Reality,Scoring,Lavatory Table,Scoring,Perimeter,Peri-Efficiency,Scoring,Total Score\n')
    baseName = result_folder + '/%06d' % success_number
    imgName = baseName + '.png'
    cv2.imwrite(imgName, img_all)
    pickleName = baseName + '.pkl'
    save_pickle((A.copy(), B_cl_re.copy(), C.copy()), pickleName)
    save_pickle(found_patterns, pklName)
    with open(csvName, 'at') as f:
        f.write(result_str)
    draw_dxf(A, B, C, ys_unique, xs_unique, lavatorys, result_folder + '/%06d.dxf' % success_number)
    success_number += 1
...
unique_blocks = list(toilet) + list(shower) + [shaft, washmach]
